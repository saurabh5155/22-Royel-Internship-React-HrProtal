...
export const ListFamilyRelation = () => {

    const [listFamilyRelation, setlistFamilyRelation] = useState([])

    const getFamilyRelation = () =>{
        axios.get(`http://localhost:4000/familyRelation`).then(res=>{
            console.log(res.data.data);
            setlistFamilyRelation(res.data.data)
        })
    }
    ...
    const deleteFamilyRelation = (id) =>{
        axios.delete(`http://localhost:4000/familyRelation/${id}`).then(res=>{
                                ...
                                <div className="table-responsive">
                                    <div id="example3_wrapper" className="dataTables_wrapper no-footer"><div className="dataTables_length" id="example3_length"><label>Show <select name="example3_length" aria-controls="example3" className><option value={10}>10</option><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option></select> entries</label></div><div id="example3_filter" className="dataTables_filter"><label>Search:<input type="search" className placeholder aria-controls="example3" /></label></div><table id="example3" className="display dataTable no-footer" style={{ minWidth: 845 }} role="grid" aria-describedby="example3_info">
                                        ...
                                        <tbody>
                                            ...
                                            listFamilyRelation.map((familyRelation) => {
                                                return (<tr role="row" className="odd">
                                                        <td>{familyRelation._id}</td>
                                                        <td>{familyRelation.relationName}</td>
                                                        <td>
													<div class="d-flex">
														<Link to={`/${Deshbord}/listFamilyRelation/updateFamilyRelation/${familyRelation._id}`} class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></Link>
														<button onClick={()=>{deleteFamilyRelation(familyRelation._id)}}  class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash" ></i></button>
													</div>												
												</td>
                                                    </tr>
