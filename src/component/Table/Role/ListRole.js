...
export const ListRole = () => {
    ...
    const deleteRole = (id) => {
        axios.delete(`http://localhost:4000/roles/${id}`).then(res => {
            console.log("Role Deleted...", res.status);
            getListRole()
        })
                                        ...
                                        <tbody>
                                            ...
                                            listRole.map((role) => {
                                                return (<tr role="row" className="odd">
                                                        ...
                                                        <td>
													<div class="d-flex">
														<Link to={`/${Deshbord}/updateRole/${role._id}`} class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></Link>
														<button onClick={()=>{deleteRole(role._id)}}  class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash" ></i></button>
													</div>												
												</td>
                                                    </tr>
