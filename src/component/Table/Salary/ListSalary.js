...
export const ListSalary = () => {
    ...
    const deleteSalary = (id) => {
        axios.delete(`http://localhost:4000/salary/${id}`).then(res => {
            getSalaryById()
        })
    }
                                    ...
                                    <div id="example3_wrapper" className="dataTables_wrapper no-footer"><div className="dataTables_length" id="example3_length"><label>Show <select name="example3_length" aria-controls="example3" className><option value={10}>10</option><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option></select> entries</label></div><div id="example3_filter" className="dataTables_filter"><label>Search:<input type="search" className placeholder aria-controls="example3" /></label></div><table id="example3" className="display dataTable no-footer" style={{ minWidth: 845 }} role="grid" aria-describedby="example3_info">
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th className="sorting" tabIndex={0} aria-controls="example" rowSpan={1} colSpan={1} aria-label="Position: activate to sort column ascending" style={{ width: '235.82px' }}>Salary Amount</th>
                                                <th className="sorting" tabIndex={0} aria-controls="example" rowSpan={1} colSpan={1} aria-label="Position: activate to sort column ascending" style={{ width: '235.82px' }}>Bonus Amount</th>
                                                <th className="sorting" tabIndex={0} aria-controls="example" rowSpan={1} colSpan={1} aria-label="Position: activate to sort column ascending" style={{ width: '235.82px' }}>SalaryDate</th>
                                        ...
                                        <tbody>
                                            ...
                                            listSalary.map((salary) => {
                                                return (<tr role="row" className="odd">
                                                        <td>{salary._id}</td>
                                                        {/* <td>{salary.userId.firstName}</td> */}
                                                        <td>{salary.salaryValue}</td>
                                                        <td>{salary.bonus}</td>
                                                        <td>{salary.salaryDate}</td>

                                                        <td>
													<div class="d-flex">
														<Link to={`/${Deshbord}/listSalary/updateSalary/${salary._id}`} class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></Link>
														<button onClick={()=>{deleteSalary(salary._id)}}  class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash" ></i></button>
													</div>												
												</td>
                                                    </tr>
